...
You will need to replace the path to the location of the folder you want to work from on your machine


```{r, message=FALSE , warning=FALSE}
wd <- "C:/Users/khowell/Desktop/smartrid/TestingSMID"

paste0(wd,"/reports") -> reports_dir
reports_dir %>% dir.create()
paste0(wd,"/taxonomy") -> taxonomy_dir
taxonomy_dir %>% dir.create()

# download the aphia IDs and 
"https://raw.githubusercontent.com/DeepSeaCRU/CRU-resources/main/Label_tree_aphiaIDs.csv" %>%
  read_csv() -> aphiaIDs

# save it as a csv table in the taxonomy directory 
 aphiaIDs%>% write_csv( paste0(taxonomy_dir,"/Label_tree_aphiaIDs.csv") )
...
allannotations_taxonomy %>%
  ...
  group_by(image_id, filename, volume, label_name, label_hierarchy) %>%
  # count the
  summarise(count = n()) %>%
  ungroup() %>%
  mutate(image_id = as.character(image_id)) -> allannotations_count

# 4) rearrange the table into useful format -------------------------------------------------------------------
# contingency table
